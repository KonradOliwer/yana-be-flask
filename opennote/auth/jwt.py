...
class JWT:
    ...
    def __init__(self, expire_at: int = None, refresh_token: UUID = None, user_id: UUID = None, algorith: str = None, signature: str = None):
    ...
    def create(cls, issued_at: int, user_id: UUID, refresh_token: UUID) -> 'JWT':
        return cls(expire_at=issued_at + JWT.TIME_TO_LIVE, refresh_token=refresh_token, user_id=user_id, algorith=JWT.SUPPORTED_ALGORITHM)
    ...
    def from_string(cls, token: str) -> 'JWT':
        try:
            ...
            return cls(algorith=header_json[JWT._ALGORITHM], expire_at=payload_json[JWT._ISSUED_AT],
                       refresh_token=UUID(payload_json[JWT._TOKE_ID]), user_id=UUID(payload_json[JWT._USER_ID]), signature=signature)
        except:
